...
export function GameScreen({ getState, dispatch, onExit }) {
  ...
  // 실제 데미지 역산이 끝나기 전까지 시간이나 웨이브 진행도로 가짜 체력을 만들지 않는다.
  // 계산 모델을 연결할 때 이 고정값만 실제 HP 비율로 교체하면 된다.
  function renderBossHealth() {
    return el('div', { class: 'boss-health-row', 'aria-label': '보스 체력 100%' }, [
      el('span', { class: 'boss-health-emblem', text: '☠' }),
      el('span', { class: 'boss-health-label', text: '보스' }),
      el('span', { class: 'boss-health-track' }, [
        el('span', { class: 'boss-health-fill', style: 'width:100%;' }),
      ]),
    ]);
  }
  ...
  // 참고 영상에서는 일반은 칸 안에 충분한 여백이 남고, 전설은 일반보다 확실히 큰
